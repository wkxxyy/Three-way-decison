...
public class Start {
    ...
    */
        ...
        minClock = vmwareList.get(0).getMinClock();//将第一个虚拟机的完成时间设置为minClock
 ...
 */
       /* for (int i = 0; i < taskList.size(); i++) {
        ...
        EVIM= CalculatePower.getCalculatePower(taskList,vmwareList,"ESTC");
        System.out.println("ESTC能耗为："+EVIM);
            ...
            for (int i = 0; i < taskList.size(); i++) {
                ...
                for (int j = 0; j < vmwareList.size(); j++) {
                    ...
                    if (currentVmware.getMinClock()>time) {//最小完成时间大于当前的时间，那么虚拟机就是在运行的
                        ...
                        if (currentVmware.getMinClock()<minClock){//获取全局最小的完成时间
                            minClock=currentVmware.getMinClock();
                            minIndex=j;
                        }//寻找最小的完成时间
